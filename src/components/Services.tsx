  ...
  return (
    <div className="flex flex-col h-full bg-[#f8fafc] pb-24 overflow-y-auto w-full">
      <div className="pt-10 pb-4 flex justify-center items-center bg-white border-b border-slate-100 sticky top-0 z-10 shadow-sm">
        <h1 className="font-medium text-slate-800">Services</h1>
      </div>
      ...
      <div className="p-4 grid grid-cols-2 gap-4">
        {products.map((product, index) => (
          ...
          >
            <div className="aspect-square w-full rounded-lg bg-slate-50 mb-3 overflow-hidden flex items-center justify-center p-2">
              <img src={product.imageUrl} alt={product.name} className="object-contain w-full h-full mix-blend-multiply" />
            </div>
            <div className="flex-1 flex flex-col">
              <h3 className="text-[13px] font-medium text-slate-700 leading-tight mb-2 flex-1">{product.name}</h3>
